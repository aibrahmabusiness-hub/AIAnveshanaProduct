...
const function_implementations_1 = require("./function-implementations");
...
function containsWrapper(input) {
    return /ap-formula-v\d+::\{/.test(input);
}
function unwrap(template) {
    return template.replace(FORMULA_REGEX, (_, _version, expr) => expr);
}
function evaluate({ expression, sampleData }) {
...
function evaluateSingleFormula({ expression, sampleData }) {
    const trimmed = expression.trim();
    if (!trimmed)
        return { result: '', error: null };
    const emptyArgError = validateFunctionArgs(trimmed);
    if (emptyArgError)
        return { result: null, error: emptyArgError };
    const { processed, vars } = preprocessExpression({ expression: trimmed, sampleData });
    try {
        return { result: (0, function_implementations_1.evaluateRaw)(processed, vars), error: null };
    }
    catch (e) {
        return { result: null, error: friendlyError(e) };
    }
}
function preprocessExpression({ expression, sampleData }) {
    const vars = {};
    let idx = 0;
    const withVars = expression.replace(/\{\{([^}]+)\}\}/g, (_, path) => {
...
function replaceInlineJsonArrays(expr, vars, idxRef) {
    let result = '';
    let i = 0;
    let inString = null;
    while (i < expr.length) {
...
function wrapStringArgs(expr) {
    const fnNames = new Set(function_registry_1.AP_FUNCTIONS.map((f) => f.name));
    let result = '';
    let pos = 0;
    while (pos < expr.length) {
        ...
        const fn = function_registry_1.AP_FUNCTIONS.find((f) => f.name === fnName);
        const closePos = findMatchingParen(expr, next.openParen);
        if (closePos === -1) {
            result += expr.slice(next.start);
            break;
        ...
        const processedArgs = argParts.map((arg, i) => {
            ...
            const shouldQuote = expectedSpec === 'string' ||
                (Array.isArray(expectedSpec) && expectedSpec.includes('string'));
...
function quoteIfBare(arg) {
    ...
    if ((trimmed.startsWith('"') && trimmed.endsWith('"')) ||
        ...
        return arg;
    if (trimmed.startsWith('__ap_'))
        return arg;
    const fnCallMatch = trimmed.match(/^([a-z_][a-z0-9_]*)\s*\(/i);
    if (fnCallMatch && function_registry_1.AP_FUNCTIONS.some((f) => f.name === fnCallMatch[1]))
        return arg;
...
function validateFunctionArgs(expr) {
    const fnNames = new Set(function_registry_1.AP_FUNCTIONS.map((f) => f.name));
    let pos = 0;
    while (pos < expr.length) {
        const next = findNextFunctionCall(expr, pos, fnNames);
...
function resolveTextVars(text, sampleData) {
    return text.replace(/\{\{([^}]+)\}\}/g, (_, path) => {
        const resolved = resolveVariable(path.trim(), sampleData);
        return resolved != null ? String(resolved) : '';
    });
}
function resolveVariable(path, sampleData) {
...
function splitArgsBySemicolon(content) {
    const args = [];
    let current = '';
    let depth = 0;
    let inString = null;
...
function findNextFunctionCall(text, fromPos, fnNames) {
    for (let i = fromPos; i < text.length; i++) {
        if (!/[a-z_]/i.test(text[i]))
            continue;
...
function findMatchingSquareBracket(text, openPos) {
    let depth = 0;
    let inStr = null;
    for (let i = openPos; i < text.length; i++) {
        const ch = text[i];
