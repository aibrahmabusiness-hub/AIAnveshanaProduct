...
# Replace openIntegrationAuthModal with showConnectionAccountsModal
js_code = """
...
import re
# Remove old openIntegrationAuthModal
text = re.sub(r'function openIntegrationAuthModal\(toolId\) \{.*?modal\.classList\.add\(\'active\'\);\s*\}', js_code, text, flags=re.DOTALL)

# Remove old event listener for closeIntegrationAuthModalBtn
text = re.sub(r'if \(document\.getElementById\(\'closeIntegrationAuthModalBtn\'\)\) \{.*?\}\s*\}\s*\}\);\s*\}', '', text, flags=re.DOTALL)
