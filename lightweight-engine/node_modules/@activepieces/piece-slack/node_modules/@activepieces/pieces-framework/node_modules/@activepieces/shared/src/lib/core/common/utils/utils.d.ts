...
export declare function truncateString({ value, maxLength, suffix }: {
    value: string;
    maxLength: number;
    suffix?: string;
}): string;
export declare function isNil<T>(value: T | null | undefined): value is null | undefined;
export declare function stringifyNullOrUndefined(data: undefined | null): string;
...
export declare function mapsAreSame<K, V>(a: Map<K, V>, b: Map<K, V>): boolean;
export declare function validateIndexBound({ index, limit, }: {
    index: number;
    limit: number;
}): number;
export declare function isManualPieceTrigger({ pieceName, triggerName }: {
    pieceName: string;
    triggerName: string;
}): boolean;
export declare function isEnumValue<T extends {
    [key: string]: string | number;
}>(enumObj: T, value: string | number): value is T[keyof T];
export type IsBase64Options = {
    allowMime?: boolean;
};
export declare function isBase64(value: string, options?: IsBase64Options): boolean;
export {};
