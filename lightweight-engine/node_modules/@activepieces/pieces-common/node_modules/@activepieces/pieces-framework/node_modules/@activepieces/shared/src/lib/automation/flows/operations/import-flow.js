...
exports._getImportOperations = _getImportOperationsForSteps;
...
function _getImportOperationsForSteps(step) {
    var _a;
    const steps = [];
    while (step) {
        if (step.nextAction) {
...
function _getImportOperationsForNotes(flowVersion, request) {
    const deleteOperations = flowVersion.notes.map(note => ({
        id: note.id,
    }));
    const addOperations = (request.notes || []).map(note => (note));
    const operations = [
        ...deleteOperations.map(operation => ({
            type: index_1.FlowOperationType.DELETE_NOTE,
            request: operation,
        })),
        ...addOperations.map(operation => ({
            type: index_1.FlowOperationType.ADD_NOTE,
            request: operation,
        })),
    ];
    return operations;
